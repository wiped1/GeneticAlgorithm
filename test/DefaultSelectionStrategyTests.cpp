...
namespace {
class IntGenotypeEvaluator : public Evaluator<Genotype<std::vector<int>>> {
public:
...
SCENARIO("EliminationStrategy removes half of a Population that has lower fitness") {
    GIVEN("A population of genotypes with integer genes with uneven number of genotypes") {
        Population<Genotype<std::vector<int>>>::CollectionType genotypes;
        genotypes.insert(std::pair<Genotype<std::vector<int>>, double>(
                Genotype<std::vector<int>>{std::vector<int>{0}}, 0));
        genotypes.insert(std::pair<Genotype<std::vector<int>>, double>(
                Genotype<std::vector<int>>{std::vector<int>{1}}, 0));
        genotypes.insert(std::pair<Genotype<std::vector<int>>, double>(
                Genotype<std::vector<int>>{std::vector<int>{2}}, 0));
        genotypes.insert(std::pair<Genotype<std::vector<int>>, double>(
                Genotype<std::vector<int>>{std::vector<int>{3}}, 0));
        genotypes.insert(std::pair<Genotype<std::vector<int>>, double>(
                Genotype<std::vector<int>>{std::vector<int>{4}}, 0));
        Population<Genotype<std::vector<int>>> pop{genotypes};

        WHEN("Population is evaluated") {
            IntGenotypeEvaluator evaluator;
            DefaultEliminationStrategy<Genotype<std::vector<int>>> selectionStrategy;
    ...
    GIVEN("A population of genotypes with integer genes with even number of genotypes") {
        ...
        genotypes.insert(std::pair<Genotype<std::vector<int>>, double>(
                Genotype<std::vector<int>>{std::vector<int>{4}}, 0));
        genotypes.insert(std::pair<Genotype<std::vector<int>>, double>(
                Genotype<std::vector<int>>{std::vector<int>{5}}, 0));
        Population<Genotype<std::vector<int>>> pop{genotypes};
    ...
    GIVEN("A population of 100 genotypes") {
        Population<Genotype<std::vector<int>>>::CollectionType genotypes;
        for (int i = 0; i < 100; i++) {
            genotypes.emplace(Genotype<std::vector<int>>{std::vector<int>{i}}, 0);
        }
        Population<Genotype<std::vector<int>>> pop {genotypes};
        WHEN("Population is evaluated") {
            IntGenotypeEvaluator evaluator;
